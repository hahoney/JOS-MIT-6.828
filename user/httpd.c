...
send_data(struct http_request *req, int fd)
{
	// LAB 6: Your code here.
        int r, length;
        struct Stat stat;
        char buf[TX_PKT_SIZE];
    
        if ((r = fstat(fd, &stat)) < 0) { 
            return r;
        }
    
        if (stat.st_size > TX_PKT_SIZE) { 
            return -E_HTTP_NOT_SEND; 
        }
        // readn returns the exact byte read from fd to buf
        if ((r = readn(fd, buf, stat.st_size)) != stat.st_size) {
            return r;
        }

        if ((r = write(req->sock, buf, stat.st_size)) < 0) {
            return r;
        }
