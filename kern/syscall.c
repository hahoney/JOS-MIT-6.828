...
sys_exofork(void)
{
	...
	// LAB 4: Your code here.
        ...
        status = env_alloc(&newenv, sys_getenvid());
        ...
        // We will pass two different ret value to parent and child env in one function
        // which is impossible to use two returns. And at the time of creataion, the function
        // return is only associated to parent env. As a result, one has to modify 
        // register (trapframe) directly. According to the asm of user program, the return value 
        // is stored in eax.
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	// LAB 4: Your code here.
        ...
        if (check < 0) { return check; }
            ...
            getenv->env_pgfault_upcall = func;
        }
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// LAB 4: Your code here.
        ...
        pte_t *src_store, *dst_store;
        int check;

        // check srcvid and dstvid
        check = envid2env(srcenvid, &srcenv, 1);
        if (check < 0) { return check; }
        check = envid2env(dstenvid, &dstenv, 1);
        if (check < 0) { return check; }

        // check srcva and dstva
        if ((srcva >= (void *)UTOP) || (dstva >= (void *)UTOP) || 
            ( ROUNDUP(srcva, PGSIZE) != srcva) || (ROUNDUP(dstva, PGSIZE) != dstva)) {
            return -E_INVAL;
        }

        // check if srcva is correctly mapped in srcenvid's address space
        srcpage = page_lookup(srcenv->env_pgdir, srcva, &src_store);
        ...
        if ((perm & PTE_W) && !(*src_store & PTE_W)) {
            return -E_INVAL;
        }

        // map pages: insert page to va 
        check = page_insert(dstenv->env_pgdir, srcpage, dstva, perm);
        if (check < 0) { 
            return -E_NO_MEM;
        }
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	// LAB 4: Your code here.
        ...
        page_remove(getenv->env_pgdir, va);
        return 0;
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
            ...
            case SYS_net_try_recv:
                return sys_net_try_recv((char *) a1, (int *) a2);
            case NSYSCALLS:
	    default:
                return -E_INVAL;
	}
